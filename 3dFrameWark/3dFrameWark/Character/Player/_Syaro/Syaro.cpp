	...
	m_forward{ Vector3::Zero }
{
	...
	if (m_gameMode == 0)
	{
		if (m_numPlayer == 1)
		{
			world_->add_camera_syaro(new_actor<DuelCamera>(world, m_position, 180.0f, m_name, m_numPlayer));
			input_.initialize(DX_INPUT_PAD1);
		}
		else if (m_numPlayer == 2)
		{
			world_->add_camera_syaro(new_actor<DuelCamera>(world, m_position, 0.0f, m_name, m_numPlayer));
	...
	else
	{
		if (m_numPlayer == 1)
		{
			world_->add_camera_syaro(new_actor<FourCamera>(world, m_position, 135.0f, m_name, m_numPlayer));
			input_.initialize(DX_INPUT_PAD1);
		}
		else if (m_numPlayer == 2)
		{
			world_->add_camera_syaro(new_actor<FourCamera>(world, m_position, 0.0f, m_name, m_numPlayer));
			input_.initialize(DX_INPUT_PAD2);
		}
		else if (m_numPlayer == 3)
		{
			world_->add_camera_syaro(new_actor<FourCamera>(world, m_position, 180.0f, m_name, m_numPlayer));
			input_.initialize(DX_INPUT_PAD3);
		}
		else if (m_numPlayer == 4)
		{
			world_->add_camera_syaro(new_actor<FourCamera>(world, m_position, -45.0f, m_name, m_numPlayer));
