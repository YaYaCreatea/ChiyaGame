...
{
	Vector3 s = Normalize(value1);
	Vector3 e = Normalize(value2);

	// 2ベクトル間の角度（鋭角側）
	float angle = acos(Angle(s, e));

	// sinθ
	float SinTh = sin(angle);

	// 補間係数
	float Ps = sin(angle * (1 - amount));
	float Pe = sin(angle * amount);

	Vector3 l_result = (Ps * s + Pe * e) / SinTh;
